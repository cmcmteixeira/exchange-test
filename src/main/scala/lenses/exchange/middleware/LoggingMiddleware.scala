package lenses.exchange.middleware
              ...
              maybeResponse =>
                maybeResponse.fold[Unit](
                  ...
                  if (response.status.isSuccess) {
                    logger.info(s"Request processed with status code ${response.status.code}")
                  } else {
                    logger.warn(s"Request processed with status code ${response.status.code}")
                })
            ))
